...
export default class parseRecurrentEvent {
  ...
  private wEvents: any[] = [];
  private full: any[] = [] ;
  public parseEvents(events: any[], start: any, end: any, top: number) {
  ...
  public parseEvent(e: any, start: any, end: any, top: number) {
      ...
      if (e.RecurrenceData.indexOf("<daily ") != -1) {
        ...
        if (arr.indexOf("dayFrequency") != -1) {
          ...
          while (loop) {
            ...
            if ((new Date(init) > end) || (rTotal > 0 && rTotal <= total) || (top > 0 && top < bindCount)) loop = false;
          }
        }
